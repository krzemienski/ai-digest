...
export async function processPodcastJob(job: Job<PodcastJobData>): Promise<void> {
  ...
  try {
    ...
    // For 25+ min episodes, use the Agent SDK for multi-turn iterative generation.
    // For shorter episodes, use the existing single-call approach.
    const useAgentMode = targetDurationMinutes >= 25;
    ...
    if (useAgentMode) {
      // Agent SDK path: autonomous multi-turn script generation
      await log.emit("script_gen", "info", `Using Agent SDK for ${targetDurationMinutes}-min episode`, {
        mode: "agent",
        model: modelId,
        storyCount: podcastItems.length,
      });

      // Build full story items with content for the agent
      const agentStories: AgentStoryItem[] = topItems.map(item => ({
        title: item.title,
        summary: item.summary,
        content: (item as Record<string, unknown>).content as string | null ?? item.summary,
        source: item.source,
        authors: Array.isArray((item as Record<string, unknown>).authors)
          ? (item as Record<string, unknown>).authors as string[]
          : [],
        topics: item.categories,
        compositeScore: item.compositeScore ?? 0,
      }));

      const agentResult = await generateScriptWithAgent(
        agentStories,
        {
          targetDurationMinutes,
          model: modelId,
      ...
      segments = agentResult.segments;
      budget.addCost(agentResult.totalCostUsd);
      ...
      await log.emit("script_gen", "info", `Agent script generated: ${segments.length} segments in ${agentResult.numTurns} turns`, {
        segmentCount: segments.length,
        agentTurns: agentResult.numTurns,
        cost: agentResult.totalCostUsd,
        elapsedMs: Date.now() - scriptStartTime,
      });

      // Agent mode: skip separate quality review — the agent's iterative process provides built-in quality
      await updateStage(episodeId, "script_gen", "done", log);
      await updateStage(episodeId, "quality_review", "done", log);
    } else {
      // Classic single-call path for short episodes (5-20 min)
      ...
      segments = parseScript(textBlock.text);
      ...
      // Stage 3: Quality Review (classic mode only)
